...
function Panchangalgetails({ panchangam }: any) {
  ...
  const getCurrentNakshatra = (panchangam: any) => {
    if (!panchangam?.nakshatras || panchangam.nakshatras.length === 0) return null;
    // Note: Adjusting index to [0] if you want current, or keeping [1] per your original logic
    const nakshatra = panchangam.nakshatras[0]; 
    return {
  ...
  return (
    <View className="flex flex-col font-inter gap-4">
      <View className="flex flex-row items-center w-full gap-2 mb-1">
        <HugeiconsIcon color="#b45309" size={32} icon={AiContentGenerator01Icon} />
      ...
      <View className="flex-row flex-wrap gap-3">
        ...
        <View className="flex-1 min-w-[45%] bg-white rounded-xl border border-neutral-200 p-4 shadow-sm">
          <Text className="text-neutral-500 text-xs uppercase font-semibold tracking-wider">Tithi</Text>
          <Text className="text-orange-600 text-lg font-bold font-merriweather-semibold mt-1">
            {tithiData?.tithi || "N/A"}
          </Text>
          <Text className="text-neutral-400 text-xs mt-1">Ends: {tithiData?.endtime || "N/A"}</Text>
        </View>

        {/* Nakshatra Card */}
        <View className="flex-1 min-w-[45%] bg-white rounded-xl border border-neutral-200 p-4 shadow-sm">
          <Text className="text-neutral-500 text-xs uppercase font-semibold tracking-wider">Nakshatra</Text>
          <Text className="text-orange-600 font-merriweather-semibold text-lg mt-1">
            {nakshatraData?.name || "N/A"}
          </Text>
          <Text className="text-neutral-400 text-xs mt-1">Ends: {nakshatraData?.endtime || "N/A"}</Text>
        </View>

        {/* Karana Card */}
        <View className="flex-1 min-w-[45%] bg-white rounded-xl border border-neutral-200 p-4 shadow-sm">
          <Text className="text-neutral-500 text-xs uppercase font-semibold tracking-wider">Karana</Text>
          <Text className="text-orange-600 text-lg font-merriweather-semibold mt-1">
            {karanaData?.name || "N/A"}
          </Text>
          <Text className="text-neutral-400 text-xs mt-1">Ends: {karanaData?.endtime || "N/A"}</Text>
        </View>

        {/* Yoga Card */}
        <View className="flex-1 min-w-[45%] bg-white rounded-xl border border-neutral-200 p-4 shadow-sm">
          <Text className="text-neutral-500 text-xs uppercase font-semibold tracking-wider">Yoga</Text>
          <Text className="text-orange-600 text-lg font-merriweather-semibold mt-1">
            {yogaData?.name || "N/A"}
          </Text>
          <Text className="text-neutral-400 text-xs mt-1">Ends: {yogaData?.endtime || "N/A"}</Text>
        </View>
      </View>
    </View>
  );
